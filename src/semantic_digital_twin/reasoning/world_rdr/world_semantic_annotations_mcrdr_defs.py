...
from krrood.entity_query_language.entity import entity, in_, infer, let
from typing_extensions import List, Set, Union
from semantic_digital_twin.world_description.world_entity import Body
from semantic_digital_twin.world_description.connections import FixedConnection, PrismaticConnection, RevoluteConnection
...
def conditions_216280842469566949273981843907929693318(case) -> bool:
    def conditions_for_world_semantic_annotations_of_type_window(case: World) -> bool:
        """Get conditions on whether it's possible to conclude a value for World.semantic_annotations  of type Window."""
        return True
    return conditions_for_world_semantic_annotations_of_type_window(case)


def conclusion_216280842469566949273981843907929693318(case) -> List[Window]:
    def world_semantic_annotations_of_type_window(case: World) -> List[Window]:
        """Get possible value(s) for World.semantic_annotations  of type Window."""
        with rule_mode():
            body = let(Body, case.kinematic_structure_entities)
            windows = infer(
                entity(
                    Window(
                        body=body
                    ),
                    in_("window", body.name.name.lower())
                ))
        return list(windows.evaluate())
    return world_semantic_annotations_of_type_window(case)
