...
from ..exceptions import HasFreeSymbolsError, NotSquareMatrixError
...
class Expression(
    ...
    @classmethod
    def from_symbolic_type(cls, data: SymbolicData):
        return cls(casadi_sx=data.casadi_sx)

    @classmethod
    def from_scalar(cls, data: NumericalScalar):
        return cls(casadi_sx=ca.SX(data))

    @classmethod
    def from_iterable(
        cls, data: Union[NumericalArray, Numerical2dMatrix, Iterable[Symbol]]
    ):
        x = len(data)
        if x == 0:
            return cls(casadi_sx=ca.SX())
        if (
        ...
        for i in range(casadi_sx.shape[0]):
            if y > 1:
                for j in range(casadi_sx.shape[1]):
                    casadi_sx[i, j] = data[i][j]
            else:
                if isinstance(data[i], Symbol):
                    casadi_sx[i] = data[i].casadi_sx
                else:
                    casadi_sx[i] = data[i]
        return cls(casadi_sx=casadi_sx)
    ...
    def tri(cls, dimension: int) -> Expression:
        return cls.from_iterable(np.tri(dimension))
...
# %% trinary logic
TrinaryFalse: Expression = Expression(0.0)
TrinaryUnknown: Expression = Expression(0.5)
TrinaryTrue: Expression = Expression(1.0)
...
def replace_with_trinary_logic(expression: Expression) -> Expression:
    ...
    op = cas_expr.op()
    if op == ca.OP_NOT:
        return ternary_logic_not(replace_with_trinary_logic(cas_expr.dep(0)))
    if op == ca.OP_AND:
