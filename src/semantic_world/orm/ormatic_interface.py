...
class ViewDAO(WorldEntityDAO, DataAccessObject[semantic_world.world_entity.View]):
    ...
    viewdao_possible_locations_id: Mapped[Optional[int]] = mapped_column(ForeignKey('ViewDAO.id', use_alter=True), nullable=True)
    multibodyviewdao_views_id: Mapped[Optional[int]] = mapped_column(ForeignKey('MultiBodyViewDAO.id', use_alter=True), nullable=True)

    possible_locations: Mapped[List[ViewDAO]] = relationship('ViewDAO', foreign_keys='[ViewDAO.viewdao_possible_locations_id]', post_update=True)
...
class FridgeDAO(ViewDAO, DataAccessObject[semantic_world.views.views.Fridge]):
    ...
    kitchendao_fridges_id: Mapped[Optional[int]] = mapped_column(ForeignKey('KitchenDAO.id', use_alter=True), nullable=True)
...
class KitchenDAO(EnvironmentViewDAO, DataAccessObject[semantic_world.views.views.Kitchen]):
    __tablename__ = 'KitchenDAO'

    id: Mapped[int] = mapped_column(ForeignKey(EnvironmentViewDAO.id), primary_key=True)




    fridges: Mapped[List[FridgeDAO]] = relationship('FridgeDAO', foreign_keys='[FridgeDAO.kitchendao_fridges_id]', post_update=True)

    __mapper_args__ = {
        'polymorphic_identity': 'KitchenDAO',
        'inherit_condition': id == EnvironmentViewDAO.id,
    }
